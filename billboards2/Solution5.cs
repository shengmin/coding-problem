 ...
 */
 ...
 public class Solution {
	...
	long Dfs(int iBoard, int nBoard, long sum) {
		...
		long without = Dfs(iBoard + 1, 0, sum);
		long with = 0L;
		
		if(nBoard < K) {
			with = Dfs(iBoard + 1, nBoard + 1, boards[iBoard] + sum);
		}
		
		long best = Math.Max(with, without);
		row[nBoard] = best;
	...
	void Run(TextReader rd) {
		...
		mem = new Dictionary<int, IDictionary<int, long>>((int)(N * 1.2)); 
