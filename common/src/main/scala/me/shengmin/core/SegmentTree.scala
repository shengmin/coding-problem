...
class SegmentTree[T](size: Int)(initializer: Int => T, aggregator: (ReadOnlySegment[T], ReadOnlySegment[T]) => T) {
  ...
  def update(itemIndex: Int)
        (base: ReadOnlySegment[T] => T, aggregator: (ReadOnlySegment[T], ReadOnlySegment[T]) => T): this.type = {
  ...
  private[this] def update(
    ...
    aggregator: (ReadOnlySegment[T], ReadOnlySegment[T]) => T
  ) {
    ...
    segment.value = aggregator(left, right)
  }
